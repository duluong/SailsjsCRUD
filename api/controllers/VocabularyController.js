...
module.exports = {
  ...
  create: function (req, res) {
    ...
    if(req.method=="POST"&& req.param("word", null)!=null){
      ...
      //       res.redirect('vocabulary/view/'+vocabulary.id);
      //     });
      // });
      ...
      req.file('avatar').upload({
          ...
          if (imgfiles.length != 0){
             ...
             console.log('file name : ' + paras.image);
          }

          Vocabulary.create(paras).exec(function createVoc(err, vocabulary){
            //res.render( 'vocabulary/view',{'model':vocabulary});
            res.redirect('vocabulary/view/' + vocabulary.id);
          });
      });
  ...
  update: function (req, res) {
    ...
    if(req.method=="GET" && id!=null){
      ...
      Vocabulary.findOne(id).exec(function(err,vocabulary){
        res.render( 'vocabulary/update',{'vocabulary':vocabulary});  
        //console.log('INNER UPDATE GET FUNCTION: ' + vocabulary.word + vocabulary.meaning); 
      });
    }
      ...
      req.file('avatar').upload({
          ...
          Vocabulary.findOne(id).exec(function(err,vocabulary){
            ...
            Vocabulary.update({id: id}, paras).exec(function(err,updated){
              res.redirect('vocabulary/view/' + vocabulary.id);
              //res.render( 'vocabulary/view',{'model':updated[0]});
              console.log('UPDATE : ' + JSON.stringify(updated[0]) ); 
            });
  ...
  delete: function (req, res) {
    ...
    Vocabulary.destroy({id: id}).exec(function(err){
      ...
      Vocabulary.find().exec(function findCB(err,vocabularies){
        ...
        res.render('vocabulary/index',{'vocabularies':vocabularies});   
      });
    });
  },
